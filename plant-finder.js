/* ============================================================
   ...
   v29 changes: html2canvas scale 2 → 1.5, JPEG quality 0.92 → 0.85.
   v30 changes: extended PDF loader phase rotation (4 phases over 7.5s).
   v31 changes: tightened phase intervals to 1.2s.
   v32 changes: phase rotation rebuilt — now driven by render progress
                instead of wall-clock timers. Time-based timers were
                getting blocked by html2canvas's main-thread work; the
                user saw the loader appear stuck. Now phases advance
                each time a page completes, so something always changes
                in step with real progress.
   v33 changes: stale "vivid-gardens-plant-plan-" PDF filename swapped
                for "plot-plant-plan-" — last live brand reference.
...
// Progress-based phase updates. Time-based setTimeout rotation doesn't work
// reliably here because html2canvas blocks the main thread during each page
// render — queued timer callbacks pile up and execute out of step. Tying
// the phase text to actual render completion guarantees the user always
// sees real progress, in step with the work.
//
// 5 pages → 5 progress points. The first phase is shown immediately;
// subsequent ones advance as each page completes its canvas render.
var renderPhases = [
"Composing your cover",
...
function onPageRendered(pageIndex) {
  // pageIndex is 0-based and represents the page that just completed.
  // We advance the phase text to describe what's about to happen next.
  if (pageIndex < renderPhases.length) {
    phaseEl.textContent = renderPhases[pageIndex];
  }
}

setTimeout(function() {
generatePDF(onPageRendered).then(function() {
...
async function generatePDF(onProgress) {
...
setTimeout(function() {
renderPagesToPDF(displayed, onProgress).then(resolve).catch(reject);
}, 400);
});
} catch (e) { reject(e); }
});
...
function renderPagesToPDF(displayed, onProgress) {
return new Promise(function(resolve, reject) {
try {
...
pdf.save("plot-plant-plan-" + slug + ".pdf");
resolve();
return;
}
var el = $(pageIds[idx]);
...
idx++;
// Notify caller that a page is done. Defer via setTimeout(0) to give
// the browser a tick to repaint the loader text before the next
// html2canvas call grabs the main thread again.
if (typeof onProgress === "function") {
setTimeout(function() { onProgress(idx); }, 0);
